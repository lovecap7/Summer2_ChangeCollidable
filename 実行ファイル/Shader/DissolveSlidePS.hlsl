...
PS_OUTPUT main(PS_INPUT input)
{
    ...
    //UVのX座標と disValue の差を元に、エッジ周辺を滑らかにフェードアウトさせるための補正値
    float xEdge = 1.0 - saturate(abs(disValue - input.uv.x));

    //0~1の値を元に、ディゾルブのしきい値を計算
    float threshold = input.uv.x + (xEdge * discval);
    ...
    edgeColor = pow(edgeColor, 20.0); //エッジの光り方を調整
    ...
    //合成
    output.color = col + edgeColor * float4(1, 1, 1, 1);
    return output;
}