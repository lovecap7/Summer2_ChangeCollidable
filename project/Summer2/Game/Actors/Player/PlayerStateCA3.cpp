...
PlayerStateCA3::PlayerStateCA3(std::weak_ptr<Player> player) :
...
void PlayerStateCA3::Update(const std::weak_ptr<Camera> camera)
{
	...
	{
		//移動
		m_player.lock()->GetRb()->SetMoveVec(GetForwardVec(camera) * kAttackMoveSpeed);
	}
	else
	{
		//少しずつ減速する
