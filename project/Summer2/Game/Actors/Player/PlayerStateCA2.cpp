...
void PlayerStateCA2::CreateAttack()
{
	...
	//武器の座標と当たり判定を攻撃に紐図ける
	m_attackC = std::make_shared<MeleeAttack>(m_leftLeg, kCA2AnimDamage, kCA2KeepFrame, kKnockBackPower, m_player);
}
...
void PlayerStateCA2::DeleteAttack(const std::shared_ptr<AttackManager>& attackManager)
{
	//攻撃判定を消す
	m_attackC->Delete();
