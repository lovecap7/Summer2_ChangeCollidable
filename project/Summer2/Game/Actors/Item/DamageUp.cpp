...
DamageUp::DamageUp(int modelHandle, Vector3 pos) :
...
void DamageUp::Entry(std::shared_ptr<ActorManager> actorManager)
{
	//アクターマネージャーに登録
	actorManager->Entry(shared_from_this());
}

void DamageUp::Exit(std::shared_ptr<ActorManager> actorManager)
{
	//アクターマネージャー解除
...
void DamageUp::Update(const Input& input, const std::unique_ptr<Camera>& camera, const std::shared_ptr<ActorManager> actorManager)
{
	//移動量を初期化
...
void DamageUp::Gravity(const Vector3& gravity)
{
...
void DamageUp::OnHitColl(const std::shared_ptr<Collidable>& other)
{
	//消滅フラグが立ってるならリターン
