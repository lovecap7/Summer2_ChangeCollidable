...
void BossKingStateBeam::Init()
{
	//次の状態を今の状態に更新
	ChangeState(shared_from_this());
}

void BossKingStateBeam::Update(const std::weak_ptr<GameCamera> camera, const std::weak_ptr<ActorManager> actorManager)
{
	//コライダブル
	auto coll = std::dynamic_pointer_cast<BossKing>(m_owner.lock());
	//死亡
	...
	//発生前まで向きを向きを変更
	else if (m_attackCountFrame < kUpdateDirFrame)
	{
		//相手のほうを向く
		coll->LookAtTarget();
	}
...
void BossKingStateBeam::UpdateBeamPos(const Vector3 pos, std::shared_ptr<Model> model, std::shared_ptr<BossKing> coll)
{
	//エフェクトの位置
...
void BossKingStateBeam::CreateAttack(VECTOR pos, const std::weak_ptr<ActorManager> actorManager)
{
	auto coll = std::dynamic_pointer_cast<BossKing>(m_owner.lock());
	//作成と参照
	...
	//ビームエフェクト
	m_beamEff = EffekseerManager::GetInstance().CreateEffect("BossKingBeamEff", m_owner.lock()->GetPos());
	//攻撃の位置
